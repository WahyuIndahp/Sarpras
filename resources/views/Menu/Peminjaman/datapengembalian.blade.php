            ...
            <div class="navbar navbar-default" role="navigation">
                <div class="container">
                    ...
                    <ul class="nav navbar-nav navbar-left">
                        ...
                        <li>
                            <h4 class="page-title">DAFTAR DATA PENGEMBALIAN</h4>
                        </li>
                    </ul>
                    ...
                    <div class="row">
                        <div class="col-sm-12">
                            <div class="card-box table-responsive">
                                <div class="m-b-30">
                                    <a href="{{url('/datareturn/create')}}" type="button" class="btn btn-custom"><i
                                ...
                                <table id="datatable-responsive"
                                    ...
                                    <thead>
                                        <tr>
                                            ...
                                            <th style="text-align:center; vertical-align: middle;">kode_pinjam</th>
                                    ...
                                    <tbody>
                                        @php
                                            $no = 1;
                                        @endphp
                                        @foreach($data as $kembali)
                                        <tr>
                                            <td scope="kembali">{{ $no++ }}</td>
                                            <td>{{ $kembali->tgl_kembali->format('d-m-Y')}}</td>
                                            <td>{{ $kembali->pinjams->kode_pinjam}}</td>
                                            <td>{{ $kembali->users->name}}</td>
                                            <td>{{ $kembali->pinjams->sarprases->nama_sarpras}}</td>
                                            <td style="text-align:right">{{ $kembali->pinjams->jml_sarpras}}</td>
                                            <td>
                                            {{-- <a href="/editpeminjaman"
                                                    class="btn btn-icon waves-effect waves-light btn-success btn-xs m-b-5">
                                                    <i class="fa fa-pencil-square-o"></i> Edit </a>
                                                    &nbsp; --}}
                                            <a href="{{url('/datareturn/'.$kembali->id )}}"
                                                    class="btn btn-icon waves-effect waves-light btn-info btn-xs m-b-5">
                                                    <i class="fa fa-info-circle"></i> Detail </a>
                                                    &nbsp;
                                            <form action="{{('/datareturn/'.$kembali->id)}}" method="post">
                                                @method('delete')    
                                                @csrf
